...
Нужен Node ≥ 22 (используется встроенный `node:sqlite`). Pi находит расширение по ключу `pi` в `package.json`.
...
**Установка** — поставить стек browser-search и запустить его контейнер Camofox:

```bash
npm run setup:browser-search   # клонирует и ставит browser-search, запускает Camofox на 127.0.0.1:9377
```

Это также выполняется автоматически при `npm install` (только код — клонирует и ставит browser-search, но не запускает контейнер; пропускается в CI или при `PI_FLEET_SKIP_SETUP=1`). Нужны `git` и `podman` либо `docker` для контейнера. По завершении печатается учётка для экспорта:

```bash
export CAMOFOX_API_KEY=<сгенерированный ключ>   # должен совпадать с запущенным контейнером Camofox
```

Это единственная учётка, нужная для беззапросного пути. `BROWSER_SEARCH_DIR` записывается автоматически, а `PI_FLEET_DIR` (для поиска команды скрейпинга по умолчанию) выставляется расширением и CLI сами.
